  ...
  try {
    ...
    for (const obj of arrayKegiatan1) {
      for (const val of incomingKegiatan1) {
        if (obj.abbr.includes(val)) {
          var kegiatan_id = arrKegiatan.find((item) => item.abbr == obj.key?.toLocaleLowerCase())?.id || '';
          result.push({
            kegiatan: [
              {
                ctg: 'A',
    ...
    for (const obj of arrayKegiatan2) {
      for (const val of incomingKegiatan2) {
        if (obj.abbr.includes(val)) {
          ...
          var kegiatan_id = arrKegiatan.find((item) => item.abbr == kegiatanB?.toLocaleLowerCase())?.id || '';
    ...
    for (const obj of arrayKegiatan3) {
      for (const val of incomingKegiatan3) {
        if (obj.abbr.includes(val)) {
          ...
          var kegiatan_id = arrKegiatan.find((item) => item.abbr == kegiatanC?.toLocaleLowerCase())?.id || '';
          // Cari material yang cocok di dalam obj.material
    ...
    for (const obj of arrayKegiatan4) {
      for (const val of incomingKegiatan4) {
        if (obj.abbr.includes(val)) {
          ...
          var kegiatan_id = arrKegiatan.find((item) => item.abbr == kegiatanD?.toLocaleLowerCase())?.id || '';
          // Cari material yang cocok di dalam obj.material
    ...
    for (const obj of arrayKegiatan5) {
      for (const val of incomingKegiatan5) {
        if (obj.abbr.includes(val)) {
          ...
          var kegiatan_id = arrKegiatan.find((item) => item.abbr == kegiatanE?.toLocaleLowerCase())?.id || '';
          // Cari material yang cocok di dalam obj.material
