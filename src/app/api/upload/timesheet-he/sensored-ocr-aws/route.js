...
export async function POST(req) {
  ...
  try {
    ...
    for (const obj of original) {
      if (['TANGGAL ID UNIT', 'TANGGAL'].includes(obj.key)) {
        const isValidDate = moment(obj.value, 'DDMMYY', true).isValid();
        if (isValidDate) {
          result = { ...result, tanggal: moment(obj.value, 'DDMMYY').format('YYYY-MM-DD') };
        } else {
          result = { ...result, tanggal: moment().format('YYYY-MM-DD') };
        }
        original = original.filter((teks) => !teks.key.startsWith('TANGGAL'));
      }

      if (obj.key == 'ID UNIT') {
        const { id: equipment_id } = arrEquipemnt.find((item) => item.abbr == obj.value) || '';
        result = { ...result, equipment_id: equipment_id };
        original = original.filter((f) => f.key !== 'ID UNIT');
      }

      if (['ID KARYAWAN', 'NO. ID'].includes(obj.key)) {
        result = { ...result, operator_id: parseInt(obj.value) || '' };
        original = original.filter((f) => f.key !== 'ID KARYAWAN');
      }

      if (obj.key == 'HM START') {
        result = { ...result, hmstart: obj.value?.replace(/[\s-]/g, '') || 0 };
        original = original.filter((f) => f.key !== 'HM START');
      }

      if (obj.key == 'HM FINISH') {
        result = { ...result, hmfinish: obj.value?.replace(/[\s-]/g, '') || 0 };
        original = original.filter((f) => f.key !== 'HM FINISH');
      }

      if (obj.key == 'BBM') {
        result = { ...result, bbm: obj.value };
        original = original.filter((f) => f.key !== 'BBM');
      }

      // if(['SEQ', 'S EQ', 'SE Q'].includes(obj.key)){
      //   result.push({
      //     seq: obj.value
      //   });
      // }
      ...
      if (obj.key == 'LOKASI') {
        result = { ...result, seq: obj.value };
      }
    }

    // ---------------------------------------------------------------------------------------------------------------
    // CHECK BLOCK OVERTIME
    const incomingOvertime = original.filter((teks) => teks.key.startsWith('L')).map((m) => m.key);
    const propOvertime = [
      { key: 'LS2', arrvalue: ['LS2', 'LSS', 'L52'] },
      { key: 'LS1', arrvalue: ['LS1', 'LSI', 'L51'] },
      { key: 'LS0', arrvalue: ['LS0', 'LSO', 'L50'] }
    ];

    const [resOvertime] = propOvertime.filter((item) => item.arrvalue.every((val) => !incomingOvertime.includes(val)));

    // result.push({ overtime: resOvertime.key?.toLocaleLowerCase(), value: 'selected' });
    result = { ...result, overtime: resOvertime.key?.toLocaleLowerCase() };
    original = original.filter((teks) => !teks.key.startsWith('LS'));
    ...
    // ---------------------------------------------------------------------------------------------------------------
    // CHECK BLOCK PENYEWA
    const incomingPenyewa = new Set(original.map((item) => item.key.replace(/\s+/g, '')));
    const arrayPenyewa = [
      { key: 'MPR', value: '' },
      { key: 'IMN', value: '' },
      { key: 'ALJ', value: '' }
    ...
    var kegiatan_id = arrKegiatan.find((item) => item.abbr == kegiatanA)?.id || '';
    result.kegiatan.push({
      ctg: 'A',
      nama: kegiatanA,
    ...
    const originKegiatan2 = original.filter((f) => f.key.startsWith('B-'));
    const incomingKegiatan2 = new Set(originKegiatan2.map((item) => item.key));
    ...
    const incomingMaterial2 = new Set(originKegiatan2.map((item) => item.key));
    ...
    var kegiatan_id = arrKegiatan.find((item) => item.abbr == kegiatanB)?.id || '';
    ...
    var kegiatan_id = arrKegiatan.find((item) => item.abbr == kegiatanC)?.id || '';
    ...
    var kegiatan_id = arrKegiatan.find((item) => item.abbr == kegiatanD)?.id || '';
    ...
    var kegiatan_id = arrKegiatan.find((item) => item.abbr == kegiatanE)?.id || '';
    ...
    const filtered = result.kegiatan?.filter((item) => {
      const hasStart = item.start !== undefined && item.start !== '';
      const hasKegiatanOrMaterial = item.kegiatan_id !== '' || item.material_id !== '';
      return hasStart && hasKegiatanOrMaterial;
    });
