...
import { useGetListMenu } from "api/menu";
...
const FilterMenu = ({
  ...
  const { data: array, dataLoading } = useGetListMenu();
    ...
    <Stack mt={2} justifyContent="flex-start" alignItems="flex-start">
      <FormControl fullWidth variant="outlined">
        <Autocomplete
          ...
          getOptionLabel={(option) => option.menu || option.nama || ""}
          sx={{ "& .MuiInputBase-root": { py: 0.9 } }}
          renderOption={(props, option) => (
            <li {...props} key={`${option.id}-${option.menu}`}>
              <Box
                sx={{
                  display: "flex",
