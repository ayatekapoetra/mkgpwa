...
const endpoint = '/customers/event-history';
...
const buildQueryString = (params = {}) => {
  ...
  Object.entries(params).forEach(([key, value]) => {
    ...
    // Never send penyewa_ids from client; backend locks to logged-in customer.
...
export const useGetCustomersEventHistory = (params) => {
...
export const downloadCustomersEventHistory = async (params, format) => {
  const query = buildQueryString({ ...params, page: undefined, perPage: undefined });
  const extension = format === 'pdf' ? 'pdf' : 'xlsx';
  const fallback = `report-event-history-customers-${params.startdate}-to-${params.enddate}.${extension}`;
