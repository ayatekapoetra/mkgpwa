...
export default function ShowLokasiKerjaScreen() {
  ...
  const { id } = useParams();
  const { lokasiKerja: initialValues, lokasiKerjaLoading: dataLoading } = useShowLokasiKerja(id);
  ...
  const onSubmitHandle = async (values) => {
    ...
    const { cabang, sts_sync, sts_sync_msg, sts_jarak, created_at, updated_at, aktif, ...cleanValues } = values;
                ...
                <Grid container spacing={2} alignItems="flex-start" justifyContent="flex-start">
                  <Grid item xs={12} sm={6} sx={{ mb: 4 }}>
                   ...
                   <Grid item xs={12} sm={6} sx={{ mb: 4 }}>
                     ...
                     />
                     {Boolean(errors.abbr) && (
                       <Typography variant="body2" color="error" gutterbound>
                         {errors.abbr}
                       </Typography>
                     )}
                   </Grid>
