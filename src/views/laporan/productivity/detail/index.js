...
import { useGetProductivityDetail } from 'api/productivity';
...
export default function ProductivityDetailScreen() {
  ...
  const penyewaId = searchParams.get('penyewa_id') || '';
  ...
  const params = useMemo(() => ({
    type, penyewa_id: penyewaId, equipment_id: equipmentId, startdate, enddate
  }), [type, penyewaId, equipmentId, startdate, enddate]);

  const { data, dataLoading, dataError } = useGetProductivityDetail(params);
    ...
    <MainCard
      title={
        <Stack direction="row" alignItems="center" spacing={2}>
          <IconButton aria-label="back" variant="dashed" color="secondary" onClick={() => router.back()}>
            <KeyboardArrowDownIcon style={{ transform: 'rotate(90deg)' }} />
          </IconButton>
          <Box>
            <Typography variant="h4">Detail {typeLabel}</Typography>
            <Typography variant="caption" color="text.secondary">
              {penyewaName} • {equipmentCode} • {formatDate(startdate)} s/d {formatDate(enddate)}
            </Typography>
          </Box>
        </Stack>
      }
          ...
          <>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6} md={3}>
                <SummaryCard
                  ...
                  icon={<Avatar sx={{ bgcolor: theme.palette[TYPE_COLORS[type]]?.main || theme.palette.primary.main, width: 40, height: 40 }}>⏱</Avatar>}
                />
              </Grid>
              <Grid item xs={12} sm={6} md={3}>
                <SummaryCard
            ...
            {data.category_totals?.length > 0 ? (
              <Card variant="outlined">
                <CardContent>
                  <Typography variant="subtitle1" gutterBottom>Breakdown per Kategori</Typography>
            ...
            {/* Detail Table per Date */}
            <DetailTable data={data.data || []} type={type} theme={theme} />
          </>
        ) : null}

        {!dataLoading && !data && !dataError ? (
          <Alert severity="info">Tidak ada data detail tersedia.</Alert>
        ) : null}
      </Stack>
    </MainCard>
  );
      ...
      <Table stickyHeader size="small">
        <TableHead>
          <TableRow>
            ...
            <TableCell sx={headerCellSx}>Tanggal</TableCell>
            <TableCell sx={headerCellSx}>Shift</TableCell>
            {isOperating ? <TableCell sx={headerCellSx}>Operator</TableCell> : null}
            <TableCell sx={headerCellSx} align="center">Items</TableCell>
            <TableCell sx={headerCellSx} align="right">Total Hours</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {data.length === 0 ? (
            <TableRow>
              <TableCell colSpan={isOperating ? 6 : 5} align="center">
                <Typography variant="body2" sx={{ py: 2 }}>Tidak ada data</Typography>
              </TableCell>
            </TableRow>
          ) : (
            data.map((entry) => {
              const isOpen = expandedRows.has(entry.date_ops);
              const entryTotal = entry.items.reduce((sum, item) => sum + (Number(item.duration_hours) || Number(item.operating_hours) || 0), 0);
            ...
            <Box sx={{ py: 1 }}>
              <Table size="small">
                <TableHead>
                  <TableRow>
                    {itemColumns.map((col) => (
                      <TableCell key={col} sx={{ ...headerCellSx, fontSize: '0.7rem' }}>{col}</TableCell>
                    ))}
                  </TableRow>
                </TableHead>
                <TableBody>
                    ...
                    <TableRow key={idx} hover>
                      {isOperating ? (
                        <>
                          ...
                          <TableCell sx={{ whiteSpace: 'nowrap' }}>{item.starttime ? formatDateTime(item.starttime) : '-'}</TableCell>
                          <TableCell sx={{ whiteSpace: 'nowrap' }}>{item.endtime ? formatDateTime(item.endtime) : '-'}</TableCell>
                          ...
                          <TableCell sx={{ whiteSpace: 'nowrap', textAlign: 'right', fontWeight: 600 }}>{formatDecimal(item.operating_hours)}</TableCell>
                        </>
                      ) : (
                        <>
                          ...
                          <TableCell sx={{ whiteSpace: 'nowrap' }}>{item.start_time ? formatDateTime(item.start_time) : '-'}</TableCell>
                          <TableCell sx={{ whiteSpace: 'nowrap' }}>{item.finish_time ? formatDateTime(item.finish_time) : '-'}</TableCell>
