...
export default function CabangDtBarChart({ data, average, loading }) {
  ...
  return (
    <Bar
      data={{
        labels,
        datasets: [{ label: 'Nilai DT (Juta)', data: values, backgroundColor: 'rgba(37,99,235,0.8)' }, ...(averageValue > 0 ? [{ type: 'line', label: 'Avg', data: labels.map(() => averageValue), borderColor: '#ef4444', borderDash: [6, 6], borderWidth: 2, pointRadius: 0, pointHoverRadius: 0 }] : [])]
      }}
