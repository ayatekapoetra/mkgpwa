...
export default function WorkOrderScreen() {
  ...
  const [apiParams, setApiParams] = useState({
    area: API_DEFAULTS.AREA,
    penyewa_id: API_DEFAULTS.PENYEWA_ID,
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (isRefreshTime && totalPages > 1) {
        ...
        const nextPage =
          currentPageRef.current >= totalPages ? 1 : currentPageRef.current + 1;
        console.log(
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Last updated: {moment().format("HH:mm:ss")} | Page {currentPage} of{" "}
          {Math.ceil((data?.pagination?.total || 1) / apiParams.perPage) || 1}
        </Typography>
        <Typography
          sx={{
            fontSize: "11px",
