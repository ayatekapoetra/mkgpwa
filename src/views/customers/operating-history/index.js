...
import {
  downloadCustomersOperatingHistory,
  useGetCustomersOperatingHistory
} from 'api/customers-operating-history';
import FilterCustomersOperatingHistory from './filter';
...
const errorMessage = (error) =>
  error?.diagnostic?.message || error?.message || 'Gagal memuat laporan Operating History.';

export default function CustomersOperatingHistoryScreen() {
  ...
  const result = useGetCustomersOperatingHistory(params);
  const pelangganNama =
  ...
  const handleDownload = async (format) => {
    try {
      ...
      saveBlob(await downloadCustomersOperatingHistory(params, format));
      enqueueSnackbar(`Operating History ${format.toUpperCase()} berhasil di-download`, {
        variant: 'success'
      });
    } catch (error) {
      ...
      >
        <Stack spacing={2} sx={{ px: 1, pb: 2 }}>
          <Alert severity="info" variant="outlined">
            Menampilkan data Operating History untuk pelanggan: <strong>{pelangganNama}</strong>
          </Alert>
          ...
          <FilterCustomersOperatingHistory
            open={openFilter}
            count={result.total}
            params={params}
            setParams={setParams}
