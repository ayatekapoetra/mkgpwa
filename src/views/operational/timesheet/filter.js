...
// ASSETS
import { Add, Building, TagUser, Buildings2, CalendarCircle, Truck } from 'iconsax-react';
...
export default function FilterTimesheet({ open, count, params, setParams, onClose, anchor = 'right' }) {
  const onResetFilter = () => {
    setParams((prev) => ({
      ...prev,
      site_id: '',
        ...
        <Stack p={1} sx={{ maxWidth: anchor == 'right' ? '400px' : '100vw' }}>
          <MainCard content={true} title={<HeaderFilter count={count} onClose={onClose} />}>
            <Grid container spacing={1} alignItems="flex-start" justifyContent="flex-start">
              ...
              <Grid item xs={12} sm={12} lg={12}>
                <FilterCabang name={'site_id'} setData={setParams} value={params.site_id} startAdornment={<Building />} />
              </Grid>
              <Grid item xs={12} sm={12} lg={12}>
                <FilterOperatorDriver name={'karyawan_id'} setData={setParams} value={params.karyawan_id} startAdornment={<TagUser />} />
              </Grid>
              <Grid item xs={12} sm={12} lg={12}>
                <FilterPenyewa
                  name={'penyewa_id'}
