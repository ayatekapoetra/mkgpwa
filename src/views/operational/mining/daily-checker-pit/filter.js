...
import { CalendarMonth, Construction, Landscape, Inventory2, Sync } from '@mui/icons-material';
...
export default function CheckerPitFilter({ open, onClose, params, setParams, anchor = 'right' }) {
  const onReset = () => {
    setParams({
      date_ops: '',
      shift_id: '',
  ...
  return (
    <SwipeableDrawer anchor={anchor} open={open} onClose={onClose} onOpen={() => {}} disableSwipeToOpen>
      <Box sx={{ width: { xs: 320, sm: 360 }, p: 2 }} role="presentation">
        <MainCard title="Filter Checker PIT" content>
          <Grid container spacing={1.5}>
            ...
            <Grid item xs={12}>
              <SelectForm
                ...
                touched={{}}
                errors={{}}
                startAdornment={<Construction fontSize="small" />}
              />
            </Grid>
            ...
            <Grid item xs={12}>
              <InputSearch
                size="md"
                label="Excavator"
                placeholder="Cari kode excavator"
                value={params.excavator_keyword || ''}
                onChange={(e) => setParams((prev) => ({ ...prev, excavator_keyword: e.target.value }))}
                startAdornment={<Construction fontSize="small" />}
              />
            </Grid>
            ...
            <Grid item xs={12}>
              <InputSearch
                size="md"
                label="PIT"
                placeholder="Cari pit"
                value={params.pit_keyword || ''}
                onChange={(e) => setParams((prev) => ({ ...prev, pit_keyword: e.target.value }))}
                startAdornment={<Landscape fontSize="small" />}
              />
            </Grid>
