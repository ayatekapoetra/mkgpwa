...
import { useCheckerStockpileDetailScope } from 'api/checker-stockpile';
...
export default function DailyCheckerStockpileDetailView() {
  ...
  const stockpile_id = searchParams?.get('stockpile_id') || '';
  const dom_id = searchParams?.get('dom_id') || '';

  const { rows, stats, dataLoading, dataError, mutate } = useCheckerStockpileDetailScope({
    date_ops,
    shift_id,
    material_id,
    stockpile_id,
  ...
  return (
    <>
      <Breadcrumbs custom heading="Detail Checker Stockpile" links={breadcrumbLinks} />

      <MainCard
        title={
          <Button component={Link} href="/daily-checker-stockpile" variant="outlined" startIcon={<ArrowLeft />}>
            Kembali
          </Button>
        }
        secondary={
      ...
      >
        {dataError && <Alert severity="warning">Gagal memuat data detail stockpile. Cek koneksi atau login ulang.</Alert>}
              ...
              <TableBody>
                ...
                {rows.map((item) => (
                  <TableRow key={item.ritase_stockpile_id || `${item.finishtime}-${item.dumptruck_id}`} hover>
                    ...
                    <TableCell>{item.dom_code || '-'}</TableCell>
