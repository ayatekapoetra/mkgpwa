  ...
  try {
    ...
    for (const obj of keyValuePairs) {
      if (obj.value != '') {
        if (obj.key === 'TANGGAL ID UNIT') {
          ...
          result = result.filter((f) => f.key != 'TANGGAL');
        }
        if (obj.key === 'TANGGAL') {
          var formatDate = obj.value.replace(/(\d{2})(\d{2})(\d{2})/, '$1-$2-20$3');
          result.push({ key: 'tanggal', value: formatDate });
    ...
    for (const obj of result) {
      ...
      if (['TUNGGU ARAHAN', 'BREAKDOWN'].includes(obj.key)) {
        ...
        result = result.filter((f) => f.key != obj.key);
      }
    }

    const prefixSet = new Set(['A-', 'B-', 'C-', 'D-', 'E-']);

    let resParsing = result
      .filter((item) => {
    ...
    return NextResponse.json({ data: [...result, ...resParsing], form: keyValuePairs });
  } catch (err) {
...
//   '2A4',
//   'D-LOADING',
//   'PEMBUATAN/PERBAIKAN MAINTENANCE STP',
//   'IMN',
//   'E ITA',
//   'MHR',
//   'B-LAND CLEARING',
...
const original = [
  ...
  { key: 'PENGAWAS MTK', value: '' },
  { key: 'PENGAWAS CONT.', value: '' },
  { key: 'AU', value: '' },
  { key: '2A4 2C1', value: '' },
  { key: '2C1', value: '' },
  { key: '2A4', value: '' },
